...
export class CustomerInvoiceGroupUpdateStrategy implements ISaveInvoiceGroupActionStrategy {
    ...
    public saveInvoiceGroup(resolve: { (result: InvoiceGroupDO): void }, reject: { (err: ThError): void }) {
        ...
        invoiceGroupRepo.getInvoiceGroupById(invoiceGroupMeta, this._invoiceGroupDO.id)
            ...
            .catch((error: any) => {
                var thError = new ThError(ThStatusCode.CustomerInvoiceGroupUpdateStrategyErrorUpdating, error);
				if (thError.isNativeError()) {
					ThLogger.getInstance().logError(ThLogLevel.Error, "error updating customer invoice group", this._invoiceGroupDO, thError);
				}
				reject(thError);
            });     
    }
