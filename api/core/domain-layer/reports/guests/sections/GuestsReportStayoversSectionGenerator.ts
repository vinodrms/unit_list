...
export class GuestsReportStayoversSectionGenerator extends AReportSectionGeneratorStrategy {
    ...
    protected getMeta(): ReportSectionMeta {
        return {
            title: this._date.toString() + " - " + "Stayovers"
        }
    }
    ...
    protected getDataCore(resolve: (result: any[][]) => void, reject: (err: ThError) => void) {
        ...
        bookingRepository.getBookingList({ hotelId: this._sessionContext.sessionDO.hotel.id }, {
            confirmationStatusList: BookingDOConstraints.ConfirmationStatuses_CountedInInventory,
            startDateLt: this._date,
            endDateGt: this._date
