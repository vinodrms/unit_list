...
export class VatTaxItemAddStrategyDO {
	name: string;
	value: number;
...
export class VatTaxItemAddStrategy extends ATaxItemActionStrategy {
	...
	protected getValidationStructure(): IValidationStructure {
		return VatTaxItemAddStrategyDO.getValidationStructure();
	}
	protected buildTaxDO(): TaxDO {
		var taxData = <VatTaxItemAddStrategyDO>this._taxObject;
		var taxDO = new TaxDO();
	...
	public saveAsync(hotelMeta: HotelMetaRepoDO, finishedSavingTaxItemCallback: { (err: ThError, updatedHotel?: HotelDO): void; }) {
		...
		return hotelRepository.addTaxesVatItemAsync(hotelMeta, this.getTaxRepoDO(), finishedSavingTaxItemCallback);
	}
}