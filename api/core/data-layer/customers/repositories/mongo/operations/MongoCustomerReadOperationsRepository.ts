...
export class MongoCustomerReadOperationsRepository extends MongoRepository {
    ...
    private buildSearchCriteria(meta: CustomerMetaRepoDO, searchCriteria: CustomerSearchCriteriaRepoDO): Object {
        ...
        if (!this._thUtils.isUndefinedOrNull(searchCriteria)) {
            ...
            mongoQueryBuilder.addExactMatch("priceProductDetails.bookingCode", searchCriteria.bookingCode);
            mongoQueryBuilder.addRegex("indexedName", searchCriteria.indexedName);
            this.appendSearchTermParams(mongoQueryBuilder, searchCriteria.searchText);
        }
    ...
    private appendSearchTermParams(mongoQueryBuilder: MongoQueryBuilder, searchTerm: string) {
        ...
        mongoQueryBuilder.addCustomQuery("$or", [
            { "indexedName": regexQuery },
            { "priceProductDetails.bookingCode": regexQuery }
        ]);
    }
}
