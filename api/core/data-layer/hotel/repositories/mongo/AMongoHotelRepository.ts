...
export abstract class AMongoHotelRepository extends MongoRepository implements IHotelRepository, IRepositoryCleaner {
	...
	public updatePaymentsPoliciesAsync(hotelMeta: HotelMetaRepoDO, paymPoliciesParams: PaymentsPoliciesRepoDO, addPaymentsPoliciesCallback: { (err: any, updatedHotel?: HotelDO): void; }) {
		this.updatePaymentsPolicies(hotelMeta, paymPoliciesParams).then((updatedHotel: HotelDO) => {
			addPaymentsPoliciesCallback(null, updatedHotel);
		}).catch((error: any) => {
			addPaymentsPoliciesCallback(error);
		});
	}
	protected abstract updatePaymentsPolicies(hotelMeta: HotelMetaRepoDO, paymPoliciesParams: PaymentsPoliciesRepoDO): Promise<HotelDO>;

	public updatePropertyDetailsAsync(hotelMeta: HotelMetaRepoDO, propertyDetails: PropertyDetailsRepoDO, updatePropertyDetailsCallback: { (err: any, updatedHotel?: HotelDO): void; }) {
		this.updatePropertyDetails(hotelMeta, propertyDetails).then((result: HotelDO) => {
			updatePropertyDetailsCallback(null, result);
		}).catch((err: any) => {
			updatePropertyDetailsCallback(err);
		});
	}
	protected abstract updatePropertyDetails(hotelMeta: HotelMetaRepoDO, propertyDetails: PropertyDetailsRepoDO): Promise<HotelDO>;
}