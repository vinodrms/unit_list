    ...
    public validateRoomCategoryStatsList(roomCategoryStatsList: RoomCategoryStatsDO[]) {
        ...
        roomCategoryStatsList.forEach((roomCategoryStats) => {
            if (roomCategoryStats.roomCategory.displayName === 'Single Room') {
                this.validateCapacity(roomCategoryStats.capacity.stationaryCapacity, 1, 1, 1, 0);
                this.validateCapacity(roomCategoryStats.capacity.rollawayCapacity, 0, 0, 0, 1);
                this.validateCapacity(roomCategoryStats.capacity.totalCapacity, 1, 1, 1, 1);
            }
            else if (roomCategoryStats.roomCategory.displayName === 'Double Room') {
                this.validateCapacity(roomCategoryStats.capacity.stationaryCapacity, 2, 1, 0, 0);
                this.validateCapacity(roomCategoryStats.capacity.rollawayCapacity, 0, 0, 0, 2);
                this.validateCapacity(roomCategoryStats.capacity.totalCapacity, 2, 1, 0, 2);
            }
            else if (roomCategoryStats.roomCategory.displayName === 'Double Double Room') {
                this.validateCapacity(roomCategoryStats.capacity.stationaryCapacity, 4, 2, 0, 0);
                this.validateCapacity(roomCategoryStats.capacity.rollawayCapacity, 0, 0, 0, 0);
                this.validateCapacity(roomCategoryStats.capacity.totalCapacity, 4, 2, 0, 0);
            }
            else if (roomCategoryStats.roomCategory.displayName === 'Twin Room') {
                this.validateCapacity(roomCategoryStats.capacity.stationaryCapacity, 2, 0, 0, 0);
                this.validateCapacity(roomCategoryStats.capacity.rollawayCapacity, 0, 0, 0, 0);
                this.validateCapacity(roomCategoryStats.capacity.totalCapacity, 2, 0, 0, 0);
            }
            else if (roomCategoryStats.roomCategory.displayName === 'Studio Room') {
                this.validateCapacity(roomCategoryStats.capacity.stationaryCapacity, 2, 1, 1, 0);
                this.validateCapacity(roomCategoryStats.capacity.rollawayCapacity, 1, 1, 1, 0);
                this.validateCapacity(roomCategoryStats.capacity.totalCapacity, 3, 2, 2, 0);
            }
            else if (roomCategoryStats.roomCategory.displayName === 'Double King Room') {
                this.validateCapacity(roomCategoryStats.capacity.stationaryCapacity, 2, 2, 1, 0);
                this.validateCapacity(roomCategoryStats.capacity.rollawayCapacity, 0, 0, 0, 0);
                this.validateCapacity(roomCategoryStats.capacity.totalCapacity, 2, 2, 1, 0);
            }
        });
    }

    public validateCapacity(capacity: ConfigCapacityDO, maxNoAdults: number, maxNoChildren: number, maxNoBabies: number, maxNoBabyBeds: number) {
        ...
        should.equal(capacity.noBabyBeds, maxNoBabyBeds);
    }
